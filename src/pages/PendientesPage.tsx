...
export function PendientesPage({ focusId, onFocusHandled }: Props) {
  ...
  async function handleCargarStock(r: Remito) {
    ...
    setNotice(null);
    try {
      // Backend stub: submit-mercaderia hoy es un no-op; igual disparamos la llamada
      // real y aprobamos el remito para reflejar el avance en el flujo.
      await remitosApi.submitMercaderia(r.id);
      await remitosApi.approve(r.id);
      setNotice(`Remito ${r.remitoNro || r.id.slice(0, 8)} enviado a stock y aprobado.`);
      await reloadRemitos();
    } catch (e) {
        ...
        {pendientes.map((r) => (
          ...
          >
            ...
            <div style={{ padding: '10px 22px 14px', flex: 1 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 11, fontWeight: 700, letterSpacing: '.4px', color: 'var(--muted-3)', padding: '10px 0 4px', borderBottom: '1px solid #eef1f6' }}>
                <span>ARTÍCULO</span>
                <span>CANT.</span>
              </div>
              <div
              ...
              >
                ...
                {(r.articulos ?? []).map((it) => (
                  <div key={it.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 12, padding: '10px 0', borderTop: '1px solid #f2f4f8' }}>
              ...
              {r.total > 0 && (
                <div style={{ textAlign: 'right', marginTop: 10, fontSize: 13, color: 'var(--muted)' }}>
                  Total: <strong style={{ color: 'var(--ink)' }}>{money(r.total)}</strong>
                </div>
              )}
            </div>
            <div style={{ padding: '16px 22px', borderTop: '1px solid #eef1f6', display: 'flex', gap: 12, justifyContent: 'flex-end' }}>
              <button
                onClick={() => setNotice('Editar ítems: PATCH /remitos/:id/items es un stub en el backend hoy, no persiste cambios todavía.')}
                title="El backend aún no implementa la edición de ítems"
                style={{
                  height: 44,
                  padding: '0 26px',
                  borderRadius: 9,
              ...
              <button
                ...
                title="POST /remitos/submit-mercaderia/:id es un stub en el backend hoy — no persiste stock aún"
                style={{
                  height: 44,
                  padding: '0 26px',
                  borderRadius: 9,
                  border: 'none',
                  background: busyId === r.id ? '#8a94a6' : 'var(--blue)',
                  color: '#fff',
                  fontWeight: 700,
                  fontSize: 14,
              ...
              >
                {busyId === r.id ? 'Cargando…' : 'Cargar Stock'}
              </button>
            </div>
          </div>
        ))}
